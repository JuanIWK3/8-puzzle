...
# Define the neighbors (possible moves)
...
def solve(game, print_steps=False):
    ...
    possible_moves = []  # (manhattan_distance, game)
    ...
    for neighbor in neighbors:
        ...
        if 0 <= x < 3 and 0 <= y < 3:
            ...
            possible_moves.append((manhattan_distance(game), game.copy()))
            # Swap back
            game[i][j], game[x][y] = game[x][y], game[i][j]

    # Sort the possible moves by their Manhattan distance
    possible_moves.sort(key=lambda x: x[0])

    # Try all possible moves
    for move in possible_moves:
        if solve(move[1], print_steps):
            return True
